class Game {
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      image(space, 0, -height * 5, width, height * 6);
      ...
      for (var plr in allPlayers) {
        ...
        if (index === player.index) {
          ...
          camera.position.x = spaceships[index - 1].position.x;
      ...
      if (player.positionY > finishLine) {
        gameState = 2;
        player.rank += 1;
        Player.updatespaceshipsAtEnd(player.rank);
        player.update();
        this.showRank();
      }
  ...
  handleObstacleCollision(index) {
    if(spaceships[index-1].collide(obstacle1) || spaceships[index-1].collide(obstacle2)) {
  ...
  handlePowerCoins(index) {
    spaceships[index - 1].overlap(powerCoins, function(collector, collected) {
      player.score += 21;
      player.update();
      //collected (coletado) é o sprite no grupo de colecionáveis que desencadeia
      //o evento
...
showRank() {
  ...
  handleSpaceShipsACollisionWithspaceshipB(index) {
    if (index === 1) {
